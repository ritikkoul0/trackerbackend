...
func GetInvestment(c *gin.Context) {
	...
	if err := config.DB.First(&investment, uint(investmentID)).Error; err != nil {
...
func UpdateInvestment(c *gin.Context) {
	...
	if err := config.DB.First(&oldInvestment, uint(investmentID)).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Investment not found"})
		return
	}
...
func LinkInvestmentToGoal(c *gin.Context) {
	...
	var investment models.Investment
	if err := config.DB.First(&investment, uint(investmentIDUint)).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Investment not found"})
		return
	}
	...
	if requestBody.GoalID != "" {
		...
		if err := config.DB.First(&goal, goalIDUint).Error; err != nil {
...
func GetInvestmentsByGoal(c *gin.Context) {
	...
	var investments []models.Investment
	if err := config.DB.Where("goal_id = ?", uint(goalIDUint)).Find(&investments).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
