...
// GetMonthlyFinancials retrieves monthly financial data for charts
func GetMonthlyFinancials(c *gin.Context) {
	// Get user_id from context
	userIDStr, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
	...
	if err := config.DB.Where("user_id = ?", uint(userID)).
		Order("month DESC").
		Limit(6).
	...
	// Convert to MonthlyData format and reverse order (oldest to newest)
	monthlyData := make([]MonthlyData, 0, len(budgets))
